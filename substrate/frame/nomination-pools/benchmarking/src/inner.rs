...
mod benchmarks {
	...
	fn pool_withdraw_unbonded(s: Linear<0, MAX_SPANS>) {
		...
		// Set the current era
		<T::StakeAdapter as StakeStrategy>::CoreStaking::set_current_era(EraIndex::max_value());
	...
	fn withdraw_unbonded_update(s: Linear<0, MAX_SPANS>) {
		...
		// Unbond the new member
		<T::StakeAdapter as StakeStrategy>::CoreStaking::set_current_era(0);
		Pools::<T>::fully_unbond(RuntimeOrigin::Signed(joiner.clone()).into(), joiner.clone())
			.unwrap();
	...
	fn apply_slash() {
		...
		for i in 1..(T::MaxUnbonding::get() + 1) {
			<T::StakeAdapter as StakeStrategy>::CoreStaking::set_current_era(i);
		...
		<T::StakeAdapter as StakeStrategy>::CoreStaking::set_current_era(T::MaxUnbonding::get() + 2);
	...
	fn apply_slash_fail() {
		...
		<T::StakeAdapter as StakeStrategy>::CoreStaking::set_current_era(1);
		...
		for i in 0..T::MaxUnbonding::get() {
			<T::StakeAdapter as StakeStrategy>::CoreStaking::set_current_era(i + 2); // +2 because we already set the current era to 1.
			assert!(Pools::<T>::unbond(
		...
		<T::StakeAdapter as StakeStrategy>::CoreStaking::set_current_era(T::MaxUnbonding::get() + 3);
		whitelist_account!(joiner);
