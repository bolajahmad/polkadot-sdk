...
pub const PARACHAIN_ID: u32 = 1101;

/// Generate the session keys from individual elements.
///
/// The input must be a tuple of individual keys (a single arg for now since we have just one key).
pub fn template_session_keys(aura: AuraId, oracle: OracleId) -> SessionKeys {
	SessionKeys { aura, oracle }
}

fn testnet_genesis(
	invulnerables: Vec<(AccountId, AuraId, OracleId)>,
			...
			keys: invulnerables
				...
				.map(|(acc, aura, oracle)| {
					(
						...
						acc,                                 // validator id
						template_session_keys(aura, oracle), // session keys
					)
				})
				.collect::<Vec<_>>(),
		},
